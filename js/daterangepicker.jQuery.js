...
jQuery.fn.daterangepicker = function(settings){
    ...
    function positionRP(){
        var less_value =  rp.parents(options.appendTo).first().offset();
        var relEl = riContain || rangeInput; //if arrows, use parent for offsets
        var riOffset = relEl.offset(),
        side = 'left',
        val = riOffset.left + less_value.left,
        ...
        rp.parent().css(side, val).css('top', riOffset.top - less_value.top + relEl.outerHeight());
    }
    ...
    function clickActions(el, rp, rpPickers, doneBtn){
        ...
        if(el.is('.ui-daterangepicker-specificDate')){
            ...
            var dateStart =(el.data('dateStart') == null)? null: ( (typeof el.data('dateStart') == 'string') ? Date.parse(el.data('dateStart')) : el.data('dateStart')());
            var dateEnd = ((el.data('dateEnd') == null)? null :(typeof el.data('dateEnd') == 'string') ? Date.parse(el.data('dateEnd')) : el.data('dateEnd')());
